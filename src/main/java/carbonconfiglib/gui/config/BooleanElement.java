...
public class BooleanElement extends ConfigElement
{
	...
	public void init() {
		...
		trueButton = addChild(new CarbonButton(0, 0, width, 18, I18n.format("gui.carbonconfig.boolean.true"), this::onTrue), isArray() ? GuiAlign.CENTER : GuiAlign.RIGHT, isArray() ? width/2 : width+(isCompound() ? 1 : 0));
		falseButton = addChild(new CarbonButton(0, 0, width+(isCompound() ? 1 : 0), 18, I18n.format("gui.carbonconfig.boolean.false"), this::onFalse), isArray() ? GuiAlign.CENTER : GuiAlign.RIGHT, isArray() ? -(width/2) : 0);
		updateData();
	}
