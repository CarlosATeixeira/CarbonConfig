...
public class EnumElement extends ConfigElement
{
	...
	public void render(MatrixStack poseStack, int x, int top, int left, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
		...
		if(isCompound()) {
			...
			GuiUtils.drawScrollingString(poseStack, font, new StringTextComponent(value), left + width - offset, top, 135, height - 2.75F, GuiAlign.LEFT, -1, 0);			
		}
		else if(isArray()) {
			GuiUtils.drawScrollingString(poseStack, font, new StringTextComponent(value), left + (canMove() ? 5 : 10), top, 140, height - 2.75F, GuiAlign.LEFT, -1, 0);
		}
		else {
			GuiUtils.drawScrollingString(poseStack, font, new StringTextComponent(value), left - 20, top, 140, height - 2.75F, GuiAlign.LEFT, -1, 0);
		}
	}
