...
public class EnumElement extends ConfigElement
{
	...
	public void init() {
		...
		if(!hasSuggestions() || isArray()) {
			if(this.isArray()) {
				addChild(new CarbonButton(0, 0, 40, 18, new TranslatableComponent("gui.carbonconfig.edit"), this::onSelect), -32);				
			}
			else {
	...
	public void render(PoseStack poseStack, int x, int top, int left, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks) {
		...
		if(isCompound()) {
			...
			GuiUtils.drawScrollingString(poseStack, font, new TextComponent(value), left + width - offset, top, 135, height - 2.75F, GuiAlign.LEFT, -1, 0);			
		}
		else if(isArray()) {
			GuiUtils.drawScrollingString(poseStack, font, new TextComponent(value), left + (canMove() ? 5 : 10), top, 140, height - 2.75F, GuiAlign.LEFT, -1, 0);
		}
		else {
			GuiUtils.drawScrollingString(poseStack, font, new TextComponent(value), left - 20, top, 140, height - 2.75F, GuiAlign.LEFT, -1, 0);
		}
	}
