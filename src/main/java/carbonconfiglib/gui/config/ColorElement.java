	...
	public void render(MatrixStack poseStack, int x, int top, int left, int width, int height, int mouseX, int mouseY, boolean selected, float partialTicks)
	{
		...
		try {
			if(isArray()) {
				...
				AbstractGui.fill(poseStack, left+187, top, left+202, top+18, Long.decode(value.get()).intValue() | 0xFF000000);
			}
			else {
				int xOff = isCompound() ? 194 : 207;
				AbstractGui.fill(poseStack, left+xOff, top-1, left+xOff+17, top+19, 0xFFA0A0A0);
				AbstractGui.fill(poseStack, left+xOff+1, top, left+xOff+16, top+18, Long.decode(value.get()).intValue() | 0xFF000000);
			}
		}
		catch(Exception e) {}
