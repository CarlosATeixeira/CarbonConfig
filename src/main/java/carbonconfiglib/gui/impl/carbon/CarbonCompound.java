...
public class CarbonCompound implements ICompoundNode, IValueActions
{
	...
	public CarbonCompound(IReloadMode mode, CompoundData data, IChatComponent name, IChatComponent tooltip, String value, String defaultValue, Function<String, ParseResult<Boolean>> isValid, Supplier<List<Suggestion>> suggestions, BiConsumer<String, IValueActions> saveAction) {
		this.mode = mode;
		this.data = data;
		this.name = name;
	...
	protected IValueActions addEntry(String value, String defaultValue, IStructuredData type, String key) {
		switch(type.getDataType()) {
			case COMPOUND: return new CarbonCompound(mode, type.asCompound(), IConfigNode.createLabel(key), createTooltip(key), value, defaultValue, T -> isValid(key, T), () -> data.getSuggestions(key, this::isSuggestionValid), (T, K) -> save(key, T));
			case LIST: return new CarbonArray(mode, type.asList(), IConfigNode.createLabel(key), createTooltip(key), value, defaultValue, T -> isValid(key, T), () -> data.getSuggestions(key, this::isSuggestionValid), (T, K) -> save(key, T));
			case SIMPLE: return new CarbonValue(mode, IConfigNode.createLabel(key), createTooltip(key), DataType.bySimple(type.asSimple()), data.isForcedSuggestion(key), () -> data.getSuggestions(key, this::isSuggestionValid), value, defaultValue, T -> isValid(key, T), (T, K) -> save(key, T));
			default: return null;
		}
	}
