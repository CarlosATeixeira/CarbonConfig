...
public class CarbonCompound implements ICompoundNode, IValueActions
{
	...
	public CarbonCompound(IReloadMode mode, CompoundData data, ITextComponent name, ITextComponent tooltip, String value, String defaultValue, Function<String, ParseResult<Boolean>> isValid, Supplier<List<Suggestion>> suggestions, BiConsumer<String, IValueActions> saveAction) {
		this.mode = mode;
		this.data = data;
		this.name = name;
	...
	private ITextComponent createTooltip(String key) {
		TextComponentString comp = new TextComponentString("");
		comp.appendSibling(new TextComponentString(key).setStyle(new Style().setColor(TextFormatting.YELLOW)));
