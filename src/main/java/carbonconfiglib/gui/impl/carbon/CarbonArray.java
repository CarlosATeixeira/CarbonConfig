...
public class CarbonArray implements IArrayNode, IValueActions
{
	...
	protected IValueActions addEntry(String value, String defaultValue, int index) {
		switch(inner.getDataType()) {
			case COMPOUND: return new CarbonCompound(mode, inner.asCompound(), name.deepCopy().appendText(" "+index+":"), tooltip, value, defaultValue, this::isValid, () -> data.getSuggestions(T -> true), T -> save(T, index));
			case LIST: return new CarbonArray(mode, inner.asList(), name.deepCopy().appendText(" "+index+":"), tooltip, value, defaultValue, this::isValid, () -> data.getSuggestions(T -> true), T -> save(T, index));
			case SIMPLE: return new CarbonValue(mode, name.deepCopy().appendText(" "+index+":"), tooltip, DataType.bySimple(inner.asSimple()), false, () -> data.getSuggestions(T -> true), value, defaultValue, this::isValid, T -> save(T, index));
			default: return null;
		}
	}
