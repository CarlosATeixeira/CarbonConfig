...
public class RegistryValue<T> extends CollectionConfigEntry<T, Set<T>>
{
	...
	public IStructuredData getDataType() {
		return ListBuilder.variants(EntryDataType.STRING, clz, this::parseEntry, T -> registry.getNameForObject(T).toString()).addSuggestions(ISuggestionProvider.wrapper(this::getSuggestions)).build(true);
	}
